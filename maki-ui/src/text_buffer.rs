...
/// A tab is one column to the width math and eight to the terminal, and a
/// bare `\r` is a line break the wrapping never sees, so both are spent here
/// rather than left to throw the caret cell off later.
...
impl TextBuffer {
    ...
    /// Replaces the whole value and parks the cursor at the start.
    ///
    ...
    pub fn set_value(&mut self, value: String) {
        self.lines = sanitize(&value).split('\n').map(str::to_string).collect();
        self.raw_x = 0;
        self.cursor_y = 0;
        self.version += 1;
    }
    ...
    /// Clamps past the end of the buffer, the way every other cursor move here
    /// clamps rather than refuses, and refuses an offset inside a character.
    ...
    /// refuses the whole edit rather than leaving half of it applied.
    ///
...
mod tests {
    ...
    #[test_case("a\r\nb", "a\nb" ; "crlf_is_one_break")]
    #[test_case("a\rb",   "a\nb" ; "lone_cr_is_a_break")]
    fn set_value_breaks_lines_the_way_a_paste_does(value: &str, expected: &str) {
        let mut buf = TextBuffer::new(String::new());
        buf.set_value(value.into());
        assert_eq!(buf.value(), expected);
    }

    /// The terminal draws a tab wider than the width math counts it, so one
    /// left in the value throws the caret cell and the wrapping off for the
    /// rest of the line.
    #[test]
    fn set_value_expands_tabs() {
        let mut buf = TextBuffer::new(String::new());
        buf.set_value("a\tb".into());
        assert_eq!(buf.value(), format!("a{TAB_SPACES}b"));
    }

    /// The cursor defaults to the end of the inserted text, so it has to count
    /// the expanded tab rather than the one byte that was asked for.
    #[test]
